...
2. **Match existing naming convention** - Within Swift Testing, both `@Test("…") func camelCase()` and `@Test func \`backtick descriptive\`()` are valid. Use whichever the neighboring tests use. Do not propose converting between them as a "fix" — that creates churn for zero correctness gain.
3. **Ask before migrating** - If user might benefit from Swift Testing, ask first. Never migrate automatically.
4. **XCTest indicators** - Keep XCTest if: `trackForMemoryLeaks`, `XCTestCase`, `setUp/tearDown` patterns exist.
5. **Search before creating doubles/helpers** - Always search for existing Stubs, Spies, Fakes, and helper methods before creating new ones.
6. **Test file structure** - Follow the standard organization: tests → helpers → doubles
7. **Variable naming in tests** - Name doubles by their role, NOT by their type (see examples below)
8. **Use `makeSUT()` pattern** - Factory method returning `(sut, collaborator)`
...
// Swift Testing — display-name + identifier style
@Test("Map throws error on non-200 HTTP response")
func map_throwsErrorOnNon200HTTPResponse() throws { }

// Swift Testing — backtick descriptive style (also accepted)
@Test
func `map throws error on non-200 HTTP response`() throws { }

@Test
func `returns nil when the resource loader returns nil`() { }
```

## Backtick-quoted test names

Swift allows function names to be backtick-quoted strings, and Swift Testing makes this a first-class convention for tests: the test runner reports the backtick string verbatim, so the identifier IS the display name. Both this and the `@Test("…") func camelCase()` style are valid; **match what the surrounding test files already use**. Do not propose converting between them.

When in doubt, grep the closest existing test file in the same module:

```bash
grep -E "@Test|func \`" Tests/**/*.swift | head
```

If you see backtick names, use backtick names. If you see `@Test("…") func camelCase()`, use that. Mixing the two styles inside the same module is the only thing to avoid.
